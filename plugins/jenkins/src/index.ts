...
function config(read: PluginConfigReader, project?: string): Cfg | null {
...
const secretEquals = (given: unknown, expected: string): boolean => {
 ...
 */
export function jobUrl(baseUrl: string, job: string): string {
  const j = job.trim().replace(/^\/+|\/+$/g, "");
  if (/^https?:\/\//i.test(j)) return j.replace(/\/+$/, "");
  if (j.startsWith("job/")) return `${baseUrl}/${j}`;
  const path = j
    .split("/")
    .filter(Boolean)
...
async function req(c: Cfg, url: string, init: RequestInit = {}): Promise<Response> {
...
const json = async <T>(c: Cfg, url: string): Promise<T> =>
  (await req(c, url)).json() as Promise<T>;
 ...
 */
async function crumbHeader(c: Cfg): Promise<Record<string, string>> {
  try {
    const issued = await json<{ crumbRequestField?: string; crumb?: string }>(
      c,
      `${c.baseUrl}/crumbIssuer/api/json`,
    );
...
async function startBuild(
  ...
  if (!location) throw new Error(`${url} returned no queue Location header (status ${res.status})`);
...
async function awaitQueue(
  ...
  for (;;) {
    const item = await json<QueueItem>(c, `${queueUrl}/api/json`);
    if (item.cancelled) {
    ...
    if (number && url) return { number, url: url.replace(/\/+$/, "") };
...
async function awaitBuild(c: Cfg, buildUrl: string, deadline: number): Promise<Build> {
  for (;;) {
    const build = await json<Build>(c, `${buildUrl}/api/json?tree=${BUILD_TREE}`);
    if (!build.building && build.result) return build;
    if (Date.now() > deadline) throw new Error(`build ${buildUrl} still running after the timeout`);
    await sleep(BUILD_POLL_MS);
  }
}

const consoleText = async (c: Cfg, buildUrl: string): Promise<string> =>
  (await req(c, `${buildUrl}/consoleText`)).text();

/* ---------- trigger events ---------- */

/** One completed build, however we learned about it. */
export interface BuildEvent {
  /** Folder path form ("folder/sub/my-job"), as configured on the instance. */
  job: string;
  number: number;
  /** SUCCESS | FAILURE | UNSTABLE | ABORTED | NOT_BUILT */
  result: string;
  /** Absolute build URL, or "" when the payload carried none. */
  url: string;
}

/** Job path out of a Jenkins URL: ".../job/folder/job/my-job/12/" -> "folder/my-job". */
export function jobFromUrl(url: string): string {
  const parts = url.split("/").filter(Boolean);
  const segments: string[] = [];
  for (let i = 0; i < parts.length - 1; i++) {
    if (parts[i] === "job") segments.push(decodeURIComponent(parts[i + 1] as string));
  }
 ...
 */
export function normalizeJenkinsWebhook(body: Record<string, unknown>): BuildEvent | null {
  ...
  if (!job || !number || !result) return null;
...
export function matchesEvent(event: string, result: string): boolean {
  switch (event) {
    case "build_finished":
      return true;
    case "build_failed":
...
const lastSegment = (job: string): string =>
  job
    .replace(/\/+$/, "")
    .split("/")
    .filter((s) => s && s !== "job")
...
export function jobMatches(configured: string, incoming: string): boolean {
...
const DEFAULT_TASK =
  "Jenkins build {{job}} #{{number}} finished as {{result}}. Find the cause and fix it.\n\n" +
  "{{url}}\n\nConsole tail:\n{{log}}";
...
const instanceKey = (project: string, instance: TriggerInstance, job: string): string =>
  [project, job, String(instance.event ?? ""), String(instance.workflow ?? "")].join("::");
...
export async function fire(
  ...
  if (previous !== undefined && event.number <= previous) return undefined;
  ...
  if (previous === undefined && !explicit) return undefined;
  if (!matchesEvent(String(instance.event ?? "build_failed"), event.result)) return undefined;
...
async function pollOnce(ctx: TriggerContext): Promise<void> {
  for (const { project, instance } of ctx.instancesOf(PLUGIN)) {
    ...
    if (!c) continue;
    try {
      const { lastCompletedBuild } = await json<{ lastCompletedBuild?: Build | null }>(
        c,
        `${jobUrl(c.baseUrl, job)}/api/json?tree=lastCompletedBuild[${BUILD_TREE}]`,
      );
      if (!lastCompletedBuild?.number || !lastCompletedBuild.result) continue;
      await fire(
        ctx,
        c,
        project,
...
const plugin: IlmariPlugin = {
  ...
  config: {
    baseUrl: {
      label: "Jenkins URL",
      description:
        "Address of your Jenkins server, without a trailing path — for example https://ci.example.com. Everything this plugin does is inert until this is set.",
      env: "JENKINS_URL",
    },
    user: {
      label: "User name",
      description:
        "The Jenkins user the API token belongs to. Its permissions decide which jobs ilmari can read and build.",
      env: "JENKINS_USER",
    },
    apiToken: {
      label: "API token",
      description:
        "The API token for that user (Jenkins user menu -> Configure -> API Token). Not your password.",
      secret: true,
      env: "JENKINS_TOKEN",
    },
    webhookSecret: {
      ...
      description:
        "A password you make up, needed only if Jenkins should notify ilmari the moment a build finishes. Jenkins must send it back in the X-Ilmari-Secret header when it calls <ilmari>/api/hooks/jenkins, or the call is rejected. Leave empty to rely on polling instead.",
      secret: true,
  ...
  nodeTypes: [
    {
      ...
      description:
        "Runs a Jenkins job and waits for the verdict: the step passes only when the build ends in SUCCESS, so it is real CI evidence rather than the agent's word. On a red build the step fails with the console tail attached, which a catch or fallback step can hand straight to an agent as {{<nodeId>.result}}. Note this builds whatever Jenkins checks out for the job — point it at a job that builds this task's branch, or it verifies the wrong code.",
      params: {
        job: {
          type: "string",
          required: true,
          description:
            "Which job to run: its name, a folder path with slashes, or a full job URL. Templates interpolate, so a branch-parameterized job can take {{task}} or {{trigger.body.<name>}}.",
          example: "cez/dje/crv/crv-fe",
        },
        params: {
          type: "string",
          description:
            "Build parameters as a JSON object; empty triggers an unparameterized build. Values are sent as strings.",
          example: '{ "BRANCH": "ilmari/fix-retry" }',
        },
        timeoutMin: {
          type: "number",
          description:
            "Fail the step if the build has not finished within this many minutes, queue time included (default 30). The build itself keeps running in Jenkins.",
          example: "45",
        },
        allowUnstable: {
          type: "boolean",
          description:
            "Treat an UNSTABLE build (built, but some tests failed) as a pass. Off by default — unstable means something is broken.",
        },
        logLines: {
          type: "number",
          description:
            "How many trailing console lines to attach when the build is not green (default 80, max 500).",
          example: "150",
        },
      },
      async run(params, ctx) {
        const c = config(ctx.pluginConfig);
        if (!c) return { ok: false, reason: "jenkins-build: the jenkins plugin has no URL configured" };
        // core renders template placeholders in string params before we see them
        const job = String(params.job ?? "").trim();
        if (!job) return { ok: false, reason: "jenkins-build: job is required" };
        ...
        if (rawParams) {
          try {
            ...
            buildParams = Object.fromEntries(
        ...
        try {
          ...
          if (ok) return { ok: true, output: summary };
          const log = tail(
            await consoleText(c, queued.url).catch(() => ""),
            Number(params.logLines ?? DEFAULT_LOG_LINES),
          );
  ...
  tools: [
    {
      ...
      description:
        "Read a Jenkins build's status and console log. Use it to find out why a pipeline is red: which stage failed, the compiler or test output, the stack trace. Returns a status line followed by the log; narrow a long log with the grep parameter instead of raising lines.",
      parameters: {
        job: {
          type: "string",
          required: true,
        ...
        build: {
          type: "string",
          description:
            "Which build: a number, or one of lastBuild, lastCompletedBuild, lastFailedBuild, lastSuccessfulBuild. Default lastBuild.",
        },
        lines: {
          type: "number",
        ...
        grep: {
          type: "string",
          description:
            "Return only lines matching this case-insensitive regular expression; the line limit then applies to the matches.",
        },
      },
      async execute(input, ctx) {
        const c = config(ctx.pluginConfig);
        if (!c) return "jenkins_console: the jenkins plugin has no URL configured";
        const job = String(input.job ?? "").trim();
        if (!job) return "jenkins_console: job is required";
        const which = String(input.build ?? "").trim() || "lastBuild";
        const url = `${jobUrl(c.baseUrl, job)}/${encodeURIComponent(which)}`;
        try {
          const build = await json<Build>(c, `${url}/api/json?tree=${BUILD_TREE}`);
  ...
  triggers: [
    {
      ...
      params: {
        ...
        event: {
          ...
          options: [...EVENT_NAMES],
          description:
            "Which outcome fires this. build_failed is FAILURE only — an ABORTED build was cancelled by someone and has nothing to fix; use build_finished to catch every outcome.",
        },
        taskTemplate: {
          type: "string",
          description:
            "The spawned task's text, which is also what the agent is asked to do. {{job}}, {{number}}, {{result}}, {{url}} and {{log}} (the console tail) interpolate. Empty gives a sensible default that already includes the log — keep {{log}} in a custom one, or the agent has to fetch it with the jenkins_console tool. Every field is also available to workflow prompts as {{trigger.body.<name>}}.",
        },
        logLines: {
          type: "number",
          description:
            "How many trailing console lines go into the task (default 80, max 500). The webhook payload cap is 64KB.",
          example: "150",
        },
      },
      ...
      async webhook(payload, ctx) {
        ...
        if (!event) return { ok: true }; // a STARTED phase or a shape we don't read

        let task: string | undefined;
        for (const { project, instance } of ctx.instancesOf(PLUGIN)) {
          if (!jobMatches(String(instance.job ?? ""), event.job)) continue;
          const c = config(ctx.pluginConfig, project);
          if (!c) continue;
  ...
  metrics: [
    {
      name: "jenkins",
      title: "Jenkins jobs",
      async collect(): Promise<MetricCard[]> {
        const ctx = liveCtx;
        if (!ctx) return [];
        const c = config(ctx.pluginConfig);
        if (!c) return [];
        ...
        const cards = await Promise.all(
          jobs.map(async (job): Promise<MetricCard | null> => {
            try {
              const { builds = [] } = await json<{ builds?: Build[] }>(
                c,
                `${jobUrl(c.baseUrl, job)}/api/json?tree=builds[number,result,duration]{0,${METRIC_WINDOW}}`,
              );
        ...
        return cards.filter((card): card is MetricCard => card !== null);
      },
    },
  ],
};
